...
Timer.prototype.pause = function () {
  ...
  let timeEleapsed = Date.now();
  this.delay = timeEleapsed - this.start;
};
...
window.onload = () => {
  ...
  levelHeading = document.querySelector("#levelHeading");
...
function drawDisplay() {
  ...
  let spawnEnemyInterval = new Interval(() => {
  ...
  let spawnLootInterval = new Interval(() => {
    spawnLoot(spawnRandomLoot());
  }, SpawnEnemyTime * 5).resume();
  let spawnObstaleInterval = new Interval(() => {
    changeRandomBackground();
    obstacleCanvasTracker++;
    spawnObstacle(
      spawnRandomObstacle(),
      randomCoords()
    );
...
function spawnRandomEnemy() {
  if (!gameControls.isGamePaused) {
    let availableEnemies = ["bandit", "archer", "guard", "wizard"];
...
function spawnRandomObstacle() {
  // * First remove old obstacles (if any)
  if (document.querySelector(`#canvas_obstacles${obstacleCanvasTracker - 1}`)) {
    arrayTracker.obstaclesArray = [];
    arrayTracker.noAttackObstacles = [];
    document
      .querySelector(`#canvas_obstacles${obstacleCanvasTracker - 1}`)
      .remove();
  }
  let obstacleTypes = ["tree", "lake", "rock"];
  let index = Math.floor(Math.random() * obstacleTypes.length);
  return obstacleTypes[index];
}

function changeRandomBackground() {
  let backgroundTypes = [
    { type: "plains", color: "green" },
    { type: "woods", color: "darkgreen" },
    { type: "cobble", color: "rgb(180, 180, 180)" },
    { type: "cave", color: "rgb(56, 54, 54)" },
  ];

  let index = Math.floor(Math.random() * backgroundTypes.length);

  ctx.fillStyle = backgroundTypes[index].color;
  ctx.fillRect(0, 0, displayOBJ.width, displayOBJ.height);

  if (backgroundTypes[index].type === 'plains') document.querySelector('html').setAttribute("style", "background:url('../images/plains.png') no-repeat center center fixed;");
  else if (backgroundTypes[index].type === 'woods') document.querySelector('html').setAttribute("style", "background:url('../images/woods.png?v1') no-repeat center center fixed;");
  else if (backgroundTypes[index].type === 'cobble') document.querySelector('html').setAttribute("style", "background:url('../images/cobble.png?v1') no-repeat center center fixed;");
  else if (backgroundTypes[index].type === 'cave') document.querySelector('html').setAttribute("style", "background:url('../images/cave.png') no-repeat center center fixed;");
}
...
addEventListener("keypress", function (event) {
  if (event.keyCode === 49 || event.keyCode === 105) {
    ...
    if (
      ...
      HTMLElements.inventoryDisplay[0].firstChild.setAttribute(
        "src",
        "../images/blank.png"
      );
    }
    ...
    if (
      ...
      HTMLElements.inventoryDisplay[1].firstChild.setAttribute(
        "src",
        "../images/blank.png"
      );
    }
    ...
    if (
      ...
      HTMLElements.inventoryDisplay[2].firstChild.setAttribute(
        "src",
        "../images/blank.png"
      );
    }
    ...
    if (
      ...
      HTMLElements.inventoryDisplay[3].firstChild.setAttribute(
        "src",
        "../images/blank.png"
      );
    }
    ...
    if (
      ...
      HTMLElements.inventoryDisplay[4].firstChild.setAttribute(
        "src",
        "../images/blank.png"
      );
    }
...
function spawnObstacle(type, coords) {
  let obstacle;
  if (type === "tree") {
    obstacle = new Obstacle("tree", coords.posX, coords.posY);
    arrayTracker.obstaclesArray.push(obstacle);
    arrayTracker.noAttackObstacles.push(obstacle);
  } else if (type === "lake") {
    obstacle = new Obstacle("lake", coords.posX, coords.posY);
    arrayTracker.obstaclesArray.push(obstacle);
  } else if (type === "rock") {
    obstacle = new Obstacle("rock", coords.posX, coords.posY);
    arrayTracker.obstaclesArray.push(obstacle);
    arrayTracker.noAttackObstacles.push(obstacle);
  }
  obstacle.draw();
...
function moveToHero() {
  if (!gameControls.isGamePaused) {
    ...
    for (let target in arrayTracker.activeAreas) {
      if (
        Math.abs(
          this.posX + this.width / 2 - arrayTracker.activeAreas[target].posX
        ) <= arrayTracker.activeAreas[target].radius &&
        Math.abs(
          this.posY + this.height / 2 - arrayTracker.activeAreas[target].posY
        ) <= arrayTracker.activeAreas[target].radius
      ) {
...
function drawObstacle() {
  ...
  let updatedCanvas = document.querySelector(
    `#canvas_obstacles${obstacleCanvasTracker}`
  );
  if (!updatedCanvas) {
    updatedCanvas = document.createElement("canvas");
    updatedCanvas.setAttribute(
      "id",
      `canvas_obstacles${obstacleCanvasTracker}`
    );
    updatedCanvas.setAttribute("width", displayOBJ.width);
          ...
          for (let j = this.posX; j > this.posX - this.range; j--) {
            for (let targetEnemy in arrayTracker.activeEnemies) {
              if (
                ...
                arrayTracker.activeEnemies[targetEnemy].currentHP -=
                  this.damage;
          ...
          if (
            ...
            if (this.type === "arrow") {
              ...
              for (let target in arrayTracker.activeFireballs) {
                if (arrayTracker.activeFireballs[target].ID === this.ID) {
                  if (
                    arrayTracker.activeFireballs[target].special === "wildfire"
                  ) {
                    let ID = "wildfire" + new Date().getTime();
                    let wildfireArea = new Area(
                      "wildfire",
...
function randomCoords() {
  let coordX = Math.floor(Math.random() * (canvas.width - 2 * 100)) + 100;
  let coordY = Math.floor(Math.random() * (canvas.height - 2 * 100)) + 100;
...
function gameOver() {
  if (!gameControls.isGameOver) {
    ...
    localStorage.clear();
    HTMLElements.gameOverPrompt.innerHTML =
      "Game Over!<br>Your score is: " + gameControls.score + "<br>";
