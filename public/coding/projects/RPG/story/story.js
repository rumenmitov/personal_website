...
let images = {
  ...
  lake: new Image(),
  rock: new Image(),
  characters_left: new Image(),
  characters_right: new Image(),
  characters_left_attack: new Image(),
  characters_right_attack: new Image(),
  shield: new Image(),
  arrow: new Image(),
  fire: new Image(),
  goddess: new Image(),
  arrow_pack_left: new Image(),
  arrow_pack_right: new Image(),
  fireball_left: new Image(),
  fireball_right: new Image(),
  wildfire_left: new Image(),
  wildfire_right: new Image(),
  healing_potion: new Image(),
  freeze_spell: new Image()
};

images.tree.src = "../images/tree.png";
images.lake.src = "../images/lake.png";
images.rock.src = "../images/rock.png";
images.characters_left.src = "../images/character_art_left.png";
images.characters_right.src = "../images/character_art_right.png";
images.characters_left_attack.src = "../images/character_art_left_attack.png";
images.characters_right_attack.src = "../images/character_art_right_attack.png";
images.shield.src = "../images/shieldIcon.png";
images.arrow.src = "../images/arrowIcon.png";
images.fire.src = "../images/fireIcon.png";
images.goddess.src = "../images/goddessIcon.png";
images.arrow_pack_left.src = "../images/arrows_pack_left.png";
images.arrow_pack_right.src = "../images/arrows_pack_right.png";
images.fireball_left.src = "../images/fireball_left.png";
images.fireball_right.src = "../images/fireball_right.png";
images.wildfire_left.src = "../images/wildfire_left.png";
images.wildfire_right.src = "../images/wildfire_right.png";
images.healing_potion.src = "../images/healing_potion.png";
images.freeze_spell.src = "../images/freezeSpell.png";
...
function levelControl() {
  ...
  if (gameControls.level === 1) {
    let pattern = ctx.createPattern(document.querySelector('#grass_map_area'), 'repeat');
    ctx.fillStyle = pattern;
    ctx.fill();
    spawnObstacle("lake", 300, 100);
    spawnObstacle("rock", 600, 50);

    for ( let i = -1000; i < 1700; i += 125) {
      spawnObstacle('tree', i, -750);
      spawnObstacle('tree', i, -625);
      spawnObstacle('tree', i, -500);
      spawnObstacle('tree', i, 500);
      spawnObstacle('tree', i, 625);
      spawnObstacle('tree', i, -750);
    ...
    for ( let j = -385; j < 385; j += 125 ) {
      spawnObstacle('tree', -1000, j);
      spawnObstacle('tree', -875, j);
      spawnObstacle('tree', -750, j);
      spawnObstacle('tree', -625, j);

      spawnObstacle('tree', -500, j);
      spawnObstacle('tree', 1200, j);

      spawnObstacle('tree', 1200, j);
      spawnObstacle('tree', 1325, j);
      spawnObstacle('tree', 1450, j);
      spawnObstacle('tree', 1600, j);
    }
    ...
    let pattern = ctx.createPattern(document.querySelector('#dark_grass_map_area'), 'repeat');
    ctx.fillStyle = pattern;
    ctx.fill();
    spawnObstacle("tree", 100, 300);
    spawnObstacle("tree", 800, 50);

    for ( let i = -1000; i < 1700; i += 125) {
      spawnObstacle('rock', i, -750);
      spawnObstacle('rock', i, -625);
      spawnObstacle('rock', i, -500);
      spawnObstacle('rock', i, 500);
      spawnObstacle('rock', i, 625);
      spawnObstacle('rock', i, -750);
    ...
    for ( let j = -385; j < 385; j += 125 ) {
      spawnObstacle('rock', -1000, j);
      spawnObstacle('rock', -875, j);
      spawnObstacle('rock', -750, j);
      spawnObstacle('rock', -625, j);

      spawnObstacle('rock', -500, j);
      spawnObstacle('rock', 1200, j);

      spawnObstacle('rock', 1200, j);
      spawnObstacle('rock', 1325, j);
      spawnObstacle('rock', 1450, j);
      spawnObstacle('rock', 1600, j);
    }
    ...
    let pattern = ctx.createPattern(document.querySelector('#stone_map_area'), 'repeat');
    ctx.fillStyle = pattern;
    ctx.fill();

    for ( let i = -1000; i < 1700; i += 125) {
      spawnObstacle('lake', i, -750);
      spawnObstacle('lake', i, -625);
      spawnObstacle('lake', i, -500);
      spawnObstacle('lake', i, 500);
      spawnObstacle('lake', i, 625);
      spawnObstacle('lake', i, -750);
    ...
    for ( let j = -385; j < 385; j += 125 ) {
      spawnObstacle('lake', -1000, j);
      spawnObstacle('lake', -875, j);
      spawnObstacle('lake', -750, j);
      spawnObstacle('lake', -625, j);

      spawnObstacle('lake', -500, j);
      spawnObstacle('lake', 1200, j);

      spawnObstacle('lake', 1200, j);
      spawnObstacle('lake', 1325, j);
      spawnObstacle('lake', 1450, j);
      spawnObstacle('lake', 1600, j);
    }
    ...
    let pattern = ctx.createPattern(document.querySelector('#dark_stone_map_area'), 'repeat');
    ctx.fillStyle = pattern;
    ctx.fill();
    spawnObstacle("lake", 300, 100);
    spawnObstacle("rock", 600, 50);
...
function handleHero() {
  if (!gameControls.isGamePaused) {
    if (heroMovement.forward) {
      ...
      arrayTracker.activeEnemies.forEach(enemy=>{
        enemy.posX -= heroOBJ.speed;
      });

      arrayTracker.activeArrows.forEach(arrow => {
        arrow.posX -= heroOBJ.speed/2;
        arrow.range -= heroOBJ.speed/2;
      });

      arrayTracker.activeFireballs.forEach(fireball => {
        fireball.posX -= heroOBJ.speed/2;
        fireball.range -= heroOBJ.speed/2;
      });

      arrayTracker.activeAreas.forEach(area => {
        area.posX -= heroOBJ.speed/2;
        drawArea(area);
      });

      availableLoot.forEach(loot => {
        loot.posX -= heroOBJ.speed/2;
        loot.draw()
      });

      document.querySelector(`#canvas_obstacles${gameControls.level}`).remove();
      let updatedObstaclesCanvas = document.createElement('canvas');
      updatedObstaclesCanvas.id = `canvas_obstacles${gameControls.level}`;
      updatedObstaclesCanvas.setAttribute("width", displayOBJ.width);
      updatedObstaclesCanvas.setAttribute("height", displayOBJ.height);
      HTMLElements.display.appendChild(updatedObstaclesCanvas);
      arrayTracker.obstaclesArray.forEach(obstacle => {
        obstacle.posX -= heroOBJ.speed/2;
        obstacle.draw();

        arrayTracker.activeEnemies.forEach(enemy => {
          if (
            (enemy.posY >= obstacle.posY &&
              enemy.posY <= obstacle.posY + obstacle.height) ||
            (enemy.posY + enemy.height >= obstacle.posY &&
              enemy.posY + enemy.height <= obstacle.posY + obstacle.height)
          ) {
            if (
              enemy.posX + enemy.width + 10 >=
              obstacle.posX &&
            enemy.posX + enemy.width + 10 <=
              obstacle.posX + obstacle.width
            ) {
              enemy.posX -= 2*heroOBJ.speed;
            }
          }
        });
      });
    ...
    if (heroMovement.up) {
      ...
      arrayTracker.activeEnemies.forEach(enemy=>{
        enemy.posY += heroOBJ.speed;
      });

      arrayTracker.activeArrows.forEach(arrow => {
        arrow.posY += heroOBJ.speed;
      });

      arrayTracker.activeFireballs.forEach(fireball => {
        fireball.posY += heroOBJ.speed;
      });

      arrayTracker.activeAreas.forEach(area => {
        area.posY += heroOBJ.speed/2;
        drawArea(area);
      });

      availableLoot.forEach(loot => {
        loot.posY += heroOBJ.speed/2;
        loot.draw()
      });
        ...
        arrayTracker.activeEnemies.forEach(enemy =>{
          if (
            ...
            (enemy.posX + enemy.width >= obstacle.posX &&
              enemy.posX + enemy.width <= obstacle.posX + obstacle.width)
          ) {
            if (
              enemy.posY >= obstacle.posY &&
              enemy.posY <= obstacle.posY + obstacle.height
            ) {
              enemy.posY += 2*heroOBJ.speed;
            }
          }
        });
      });
...
function drawHero() {
  ...
  if (this.direction === "left") {
    heroImage = images.characters_left;
    heroWidthHelper = 10;
  } else {
    heroImage = images.characters_right;
    heroWidthHelper = 12;
  }
  let abilityImage;
  if (heroType == 1) {
    abilityImage = images.shield;
  } else if (heroType == 2) {
    abilityImage = images.arrow;
  } else if (heroType == 3) {
    abilityImage = images.fire;
  } else if (heroType == 4) {
    abilityImage = images.goddess;
  }
  ...
  if (this.isAttacking) {
    if (this.direction === "left") {
      heroImage = images.characters_left_attack;
    } else {
      heroImage = images.characters_right_attack;
    }
  }
...
function drawEnemy() {
  if (!gameControls.isGamePaused) {
    ...
    if (this.direction === "right") {
      enemyImage = images.characters_right;
    } else {
      enemyImage = images.characters_left;
    }
    if (this.isAttacking) {
      if (this.direction === "left") {
        enemyImage = images.characters_left_attack;
      } else {
        enemyImage = images.characters_right_attack;
      }
    }
    if (this.effects === "freeze") {
...
function drawArrow() {
  ...
  let arrowImage;
  if (this.direction === "left") {
    arrowImage = images.arrow_pack_left;
  } else {
    arrowImage = images.arrow_pack_right;
  }
...
function drawFireball() {
  ...
  if (this.direction === "left") {
    if (this.special === "wildfire") {
      fireballImage = images.wildfire_left;
    } else {
      fireballImage = images.fireball_left;
    }
  } else {
    if (this.special === "wildfire") {
      fireballImage = images.wildfire_right;
    } else {
      fireballImage = images.fireball_right;
    }
  }
  if (this.special === "normal") {
    localCtx.drawImage(
...
function drawObstacle() {
  let obstacleImage;
  if (this.type === "tree") {
    obstacleImage = images.tree;
  } else if (this.type === "lake") {
    obstacleImage = images.lake;
  } else if (this.type === "rock") {
    obstacleImage = images.rock;
  }
  ...
  if (this.currentHP <= 0) {
    for (let targetEnemy in arrayTracker.activeEnemies) {
      if (arrayTracker.activeEnemies[targetEnemy].ID === this.ID) {
        if (document.getElementById(`canvas_${this.ID}`)) document.getElementById(`canvas_${this.ID}`).remove();
...
function drawLoot() {
  ...
  let lootImage;
  if (this.type === "healing_potion") {
    lootImage = images.healing_potion;
  } else if (this.type === "freezeSpell") {
    lootImage = images.freeze_spell;
  }
  localCtx.drawImage(
    lootImage,
    this.posX,
